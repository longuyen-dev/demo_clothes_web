...
const storageKey = "Devise-Storage";
const storageRoleKey = "Devise-roles";

const defaultOptions: DeviseOptions = {
...
class Devise {
  ...
  constructor(options: DeviseOptions) {
    ...
    this.apiAuthUrl = `${this.apiUrl}${
      this.options.authUrl.base ? options.authUrl.base : "/auth"
    }`;
    ...
    this.signInUrl = `${this.apiAuthUrl}${
      this.options.authUrl.signIn ? this.options.authUrl.signIn : "/sign_in"
    }`;
    this.signOutUrl = `${this.apiAuthUrl}${
      this.options.authUrl ? this.options.authUrl.signOut : "/sign_out"
    }`;
  ...
  test() {
    Axios.get(this.signInUrl)
      ...
      .catch(error => {
        if (error.response) {
          console.log("Connexion success");
          this.debugIfActive("Devise Config", this)
        } else {
  ...
  signIn(email: string, password: string) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.log(signInResponse.headers);
